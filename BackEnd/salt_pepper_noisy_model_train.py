...
def train_salt_pepper_model(noisy_dir="salt_pepper_noisy", num_epochs=30, batch_size=16):
    """Train a denoising autoencoder model for Salt & Pepper noise"""
    print("\nTraining model for Salt & Pepper noise removal...")
    ...
    for epoch in range(num_epochs):
        ...
        # Save a checkpoint every 5 epochs (adjusted for 30 total epochs)
        if (epoch + 1) % 5 == 0:
            torch.save({
                'epoch': epoch,
                ...
                'val_loss': val_loss
            }, f"models/salt_pepper_denoising_epoch_{epoch + 1}.pth")

    # Save the final model
    torch.save(model.state_dict(), f"models/salt_pepper_denoising_final.pth")
    ...
    plt.legend()
    plt.savefig(f"models/salt_pepper_loss_curve.png")
    plt.close()
...
def visualize_results(model, noisy_dir="salt_pepper_noisy", num_samples=5):
    ...
    plt.tight_layout()
    plt.savefig(f"results/denoising_comparison_salt_pepper.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    if not os.path.exists("salt_pepper_noisy") or len(glob.glob("salt_pepper_noisy/*.png")) == 0:
        ...
        # Train the model
        salt_pepper_model = train_salt_pepper_model()  # Default is now 30 epochs

        # Visualize results
        visualize_results(salt_pepper_model)

        print("Training complete! Salt & Pepper denoising model saved in the 'models' directory.")
